...
def plot_all_agents_rewards_smooth(agent_rewards, scenario, output_dir):
    ...
    plt.xlabel('Episode')
    plt.ylabel('Agent Reward')
    plt.title(f'All Agents Reward Progress (agent + avdversary) - {scenario}- smothed')
    
    all_rewards = np.concatenate([rewards for rewards in agent_rewards.values()])
    for agent_name, rewards in agent_rewards.items():
        reward_MA = moving_average(rewards,100)
        plt.plot(range(1, len(reward_MA) + 1), reward_MA, label=f'Agent {agent_name}')

    plt.yticks(calculate_tick_range(all_rewards))
    plt.legend()
    plt.grid()
    save_plot(plt, f'All_Agents_Reward_Progress_{scenario}_smooth_100.png', output_dir)
    ...
    for agent_name, rewards in agent_rewards.items():
        reward_MA = moving_average(rewards,150)
        plt.plot(range(1, len(reward_MA) + 1), reward_MA, label=f'Agent {agent_name}')

    plt.yticks(calculate_tick_range(all_rewards))
    plt.legend()
    plt.grid()
    save_plot(plt, f'All_Agents_Reward_Progress_{scenario}_smooth_150.png', output_dir)
    
    for agent_name, rewards in agent_rewards.items():
        rewards_series = pd.Series(rewards)
        smoothed_rewards = rewards_series.rolling(window=100).mean()
        plt.plot(range(1, len(smoothed_rewards) + 1), smoothed_rewards, label=f'Agent {agent_name}')
    ...
    plt.legend()
    plt.grid()
    save_plot(plt, f'All_Agents_Reward_Progress_{scenario}_smooth_100_new.png', output_dir)
    # Open a text file in write mode
    with open("reward_data.txt", "w") as file:
        # Loop through each agent's rewards
        for agent_name, rewards in agent_rewards.items():
            reward_MA = moving_average(rewards, 100)
            # Write the data to the file
            file.write(f"Agen100t {agent_name}:\n")
            for i, reward in enumerate(reward_MA):
                file.write(f"Step {i+1}: {reward}\n")

    with open("reward_data2.txt", "w") as file:
        # Loop through each agent's rewards
        for agent_name, rewards in agent_rewards.items():
            reward_MA = moving_average(rewards, 150)
            # Write the data to the file
            file.write(f"Agen150t {agent_name}:\n")
            for i, reward in enumerate(reward_MA):
                file.write(f"Step {i+1}: {reward}\n")
...
def plot_everything(output_dir, scenario, k, score_history_100, score_history, epsiode_mean_agent_rewards):
    # Plot results for different subpolicies
            ...
            plot_all_agents_rewards_smooth(epsiode_mean_agent_rewards, f"{scenario} - {k}", output_subdir)
